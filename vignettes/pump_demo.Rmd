...
# Demo of using the PUMP package for a simple power calculation


To illustrate the `PUMP` package we conduct a power analysis for a blocked cluster-randomized RCT (this is a three level design with the random assignment at level two).
...
The `PUMP` package also allows specifying different pairwise correlations between the different outcomes via a matrix of $\rho$s rather than a single $\rho$; also for simplicity, we do not do that here.
...
We can use `PUMP` to calculate MDESes as well as power.

...
Due to this, the designers blocked the schools by district, but also by school type and year of roll-out.  After having to drop some schools due to various reasons, they were left with 29 high schools and 29 middle schools across 21 random assignment blocks.
...
The levels of this design are the random assignment (RA) blocks at level 3, schools as clusters at level 2, and students nested within the schools at level 1.  RA blocks are nested in districts, but if we use fixed effects for the RA blocks, as the authors of the published report did, then we can ignore this 4th level of nesting.
...
<!-- KH comment: there are nested parantheticals here! -->
The report has 12 outcomes of interest, grouped into 4 groups (Attendance, Behavior, Course performance (the "ABC's"), and an ABC composite measure of an indicator of whether a student is above given thresholds on the first three measures).
<!-- KH comment: this what?-->
<!-- how does separate analysis control family-wise error? -->
Given this, we might do power analysis separately for each group to control family-wise error.^[We note that there are different guidelines for when to adjust for multiple outcomes in education studies. For example, Schochet (2008) recommends organizing primary outcomes into domains, conducting tests on composite domain outcomes, and applying multiplicity corrections to composites across domains. The What Works Clearinghouse applies multiplicity corrections to findings within the same domain rather than across different domains. Our methods apply to either case. In this paper, the word “outcome” refers to either a single outcome or an outcome domain, and the paper focuses on any situation in which an analyst would apply adjustments to account for multiple outcomes.] 
...
p <- pump_power( design = "d3.2_m3ff2rc",
...
For a given design, one might analyze the resulting data differently.  For example, we could use a fixed effect model, as above, or possibly a random effects model. We specify this choice with the `design` parameter:

```{r}
p <- pump_power( design = "d3.2_m3rr2rc",
            MTP = "Bonferroni",
            MDES = rep( 0.10, 3 ),
            M = 3,
            J = 3, # number of schools/block
...
m <- pump_mdes( design = "d3.2_m3ff2rc",
           MTP = "Holm",
...
p <- pump_power( design = "d2.1_m2fc",
            MTP = "Holm", MDES = 0.05, 
            M = 3, nbar = 258, J = smp$`Sample size`, K = 21,
            Tbar = 0.50, alpha = 0.05, numCovar.1 = 5, numCovar.2 = 1,
            R2.1 = 0.1, R2.2 = 0.7, ICC.2 = 0.05, ICC.3 = 0.4, rho = 0.4 )
```

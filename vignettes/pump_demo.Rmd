...
```{r samplesizeverify}
p_check <- update( smp, type = "power", tnum = 500,
                   long.table = TRUE )
```
...
p2 <- update( p_check,
              MTP = c( "BF", "HO", "WY-SD" ),
              tnum = 500, B = 100, parallel.WY.cores = 2 )
plot( p2 )
```

To speed up computation, we could set an additional option, `parallel.WY.cores`, to the number of desired cores to parallelize the computation.
...
```{r}
plot( grid, var.vary = "ICC.2", power.definition = "min1" )
```

The built-in plot function visualizes one parameter varying at a time.
To visualize the variation in two parameters, we can construct our own plot.
...
```{r, echo=FALSE, fig.height = 2.25, warnings=FALSE }

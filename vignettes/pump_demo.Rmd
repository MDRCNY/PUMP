...
The second row has our power with the testing correction.  We see power is substantially deminished, but we still have 86% chance of rejecting at least 1 null of our 3 outcomes: while our study will not be well powered for any individual effect, it is more powered than we might expect to detect some effect, even when using the very conservative correction procedure of Bonferroni.
...
For a given design, one might analyze the resulting data differently.  For example, we could use a fixed effect model, as above, or possibly a random effects model. We specify this with the `design` parameter:

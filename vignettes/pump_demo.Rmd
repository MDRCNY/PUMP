...
# Overview of PUMP Package

PUMP allows for power calculations for a wide variety of common experimental contexts.
A context is specification of the design followed by specification of the planned statistical model that will be used to analyze the resulting data.
You can see supported designs (along with PowerUp! names) via the `pump_info()` method:

```{r}
pump_info( "context" ) %>%
  dplyr::select( -Design, -Model, -Comment ) %>%
  knitr::kable()
```

For a given design, the researcher needs to specify the listed design parameters which describe the context being investigated.  The parameters are:

```{r}
pump_info("parameters" ) %>%
  knitr::kable()
```

The PUMP package accounts for multiple testing correction across multiple outcomes.  It can do this for the following adjustment procedures:

```{r}
pump_info( "adjustment" )
```

With multiple outcomes come multiple definitions of power. See our paper for more complete discussion, but the list of powers are:
...
The critical methods for the package are:

 * `pump_mdes()`, `pump_sample()`: These methods will calculate the MDES for a given level of power, or the needed sample size to achieve a given level of power.
 * `pump_power()`: This core method calculates all the defined powers of a given design.
 * `pump_mdes_grid()`, `pump_sample_grid()`, and `pump_power_grid()`: These allow for systematically calling the three core methods across all combinations of a given set of parameters. Use these to explore sensitivity of a design to different values of the design parameters.
 * `update()` and `update_grid()`: Use update on a prior result to change a few parameters for that result.  `update_grid()` will promote a single analysis to a grid, allowing for easy sensitivity checks.
 * `plot()`: Plot results from the above methods to get easy visualizations.
 
We next illustrate all of these methods in the case study below.

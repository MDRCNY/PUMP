...
#'
#' @param comment TRUE/FALSE, prints out description of
#'                each design or method
#'
#' @export
pump_info <- function( comment = TRUE) {
    ...
    design <- tidyr::separate(design,
      .data$Code, into = c("Design", "Model"), remove = FALSE, sep = "_" )
    ...
    params <- tibble::tribble( ~ Parameter, ~ Description,
      ...
      "J",          "number of level 2 units per
                     level 3 unit (schools per district)",
...
calc_df <- function(design, J, K, nbar, 
    ...
    if(validate & df <= 0)
    {
        stop('Invalid design parameters resulting in 
...
#'       design = design, params.list = params,

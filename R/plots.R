...
plot.pumpgridresult.mdes <- function( 
    x, power.definition = NULL, var.vary, lines = lines, include.title = include.title, ...  
)
{
    M <- params(x)$M
    dots <- list( ... )
...
plot.pumpgridresult.sample <- function( 
    x, power.definition = NULL, var.vary, lines = lines, include.title, ...  
) {
    dots <- list( ... )
    ...
    if ( res$multiPower ) {
        grid.plot <- grid.plot + 
            ggplot2::facet_wrap( ~ power.definition, nrow = dots$nrow, ncol = dots$ncol )
    }
...
plot.pumpgridresult <- function( 
    x, power.definition = NULL, var.vary = NULL, lines = TRUE, include.title = FALSE, ... 
)
{
    # validation
